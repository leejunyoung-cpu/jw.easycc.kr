        ...
        <div id="ListView" style="display:none">
            <div class="tblst0">
                        ...
                        <asp:Repeater ID="repList" runat="server" EnableViewState="false">
                            <ItemTemplate>
                                <tr class="tablelist">
                                    ...
                                    <td>
                                        <select name="menusort" class="type_small" id="menusort_<%# Server.HtmlEncode(Eval("MenuSort").ToString())%>" onchange="fnChgSort('menusort', this, '<%# Server.HtmlEncode(Eval("MenuSort").ToString())%>');"></select>
				                        <script type="text/javascript">
                                    ...
                                    <td>
                                        <button type="button" class="btn_b01" onclick="javascript:fnUpdAdminMenu('메뉴 수정','<%# Server.HtmlEncode(Eval("MenuGroupNo").ToString())%>', '<%# Server.HtmlEncode(Eval("MenuNo").ToString())%>');">조회</button>
                                        <button type="button" class="btn_b01 gray" onclick="javascript:fnDelAdminMenu('<%# Server.HtmlEncode(Eval("MenuNo").ToString())%>','<%# Server.HtmlEncode(Eval("MenuName").ToString())%>');">삭제</button>
                                    </td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                ...
                <div style="margin-top:10px;text-align:center">
                    <button type="button" class="btn_b01" onclick="javascript:fnUpdAdminMenu('메뉴 등록','','');">메뉴추가</button>
                    <button type="button" class="btn_b01 gray" onclick="javascript:fnUpdAdminMenuAll();">수정반영</button>
                </div>
            </div>
        </div>
    </div>
