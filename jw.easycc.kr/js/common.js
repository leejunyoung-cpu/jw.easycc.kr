...
/*							디자인 Alert 실행함수							   */
/***************************************************************************/
function fnDefaultAlert(msg, type = "error", closeFunction = "", closeParam = null, title = "EASYCC") {
    Swal.fire({
...
function fnDefaultAlertFocus(msg, focusId = "", type = "error", closeFunction = "", closeParam = null, title = "EASYCC") {
    Swal.fire({
        icon: type, //error,info,success,warning,question
        title: title,
        html: msg,
...
/*							디자인 Confirm 실행함수						       */
/***************************************************************************/
function fnDefaultConfirm(msg, trueFunction = "", trueParam = null, falseFunction = "", falseParam = null, type = "question", title = "EASYCC") {
    Swal.fire({
        title: title,
        html: msg,
        icon: type, //error,info,success,warning,question
        ...
        if (result.isConfirmed) {
            ...
            if (falseFunction) {
                if (falseParam !== null) {
                    window[falseFunction](falseParam);
                } else {
...
//팝업형태
//1.주소를 하나만 받을때
function fnOpenAddressOnlyOne(type) {
    ...
    new daum.Postcode({
        oncomplete: function (data) {
            ...
            fnSetAddressOnlyOne(type, post, addr, extraAddr);
        }
    }).open(objOpenParam);
}

function fnSetAddressOnlyOne(type, post, addr, addrDtl) {
    var strFullAddr = "";
    if ($("#" + type + "AddrPost").length > 0) {
        $("#" + type + "AddrPost").val(post);
    }

    $("#" + type + "Addr").val(addr + " " + addrDtl);
}
...
function fnOpenAddressLayer(type) {
    ...
    if ($("#DivDaumAddrWrap").length == 0) {
        $('body').append(
            "<div id=\"DivDaumAddrWrap\"><div id=\"DivDaumAddr\"><img src=\"/images/p_close3.png\" id=\"BtnDaumAddrClose\" alt=\"닫기 버튼\"></div></div>");
    }
...
function fnOpenDaumPostcodeLayer(type) {
    ...
    new daum.Postcode({
        oncomplete: function (data) {
            ...
            fnSetAddressLayer(type, post, addr, "", sido, sigungu, dong);
...
// 그리드 셀 날짜 형식을 yyyy-mm-dd로 변환하는 포맷터 함수
function dateFormatter(row, cell, value, columnDef, dataContext) {
    if (!value) return ""; // 값이 없으면 빈 문자열 반환

    // value가 숫자일 수 있으므로 문자열로 변환
    var dateStr = value ? value.toString() : "";

    // yyyymmdd 형식을 yyyy-mm-dd로 변환
    var year = dateStr.substring(0, 4);  // 첫 4자리: 연도
    var month = dateStr.substring(4, 6); // 중간 2자리: 월
    var day = dateStr.substring(6, 8);   // 마지막 2자리: 일
